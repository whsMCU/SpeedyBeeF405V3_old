...
{
	while(!__HAL_SPI_GET_FLAG(&hspi2, SPI_FLAG_TXE));
	HAL_SPI_Transmit(&hspi2, buffer, len, 100);
}
...
static void sdcard_select(void)
{
    ...
    gpioPinWrite(_PIN_SDCARD_CS, _DEF_LOW);
}
...
static void sdcard_deselect(void)
{
    ...
    gpioPinWrite(_PIN_SDCARD_CS, _DEF_HIGH);
}
...
static void sdcard_reset(void)
{
    ...
    if (sdcard.state >= SDCARD_STATE_READY) {
        ...
        SPI_Set_Speed(_DEF_SPI2, SPI_BAUDRATEPRESCALER_32);
    }
...
static uint8_t sdcard_sendCommand(uint8_t commandCode, uint32_t commandArgument)
{
    ...
    SPI_TxBuffer(command, sizeof(command));
...
static bool sdcard_validateInterfaceCondition(void)
{
    ...
    if (status == (SDCARD_R1_STATUS_BIT_ILLEGAL_COMMAND | SDCARD_R1_STATUS_BIT_IDLE)) {
        ...
        SD_RxDataBlock(ifCondReply, sizeof(ifCondReply));
        /*
...
static bool sdcard_readOCRRegister(uint32_t *result)
{
    ...
    SD_RxDataBlock(response, sizeof(response));
...
static sdcardReceiveBlockStatus_e sdcard_receiveDataBlock(uint8_t *buffer, int count)
{
	...
	do {
		dataToken = SPI_RxByte();
	} while((dataToken == 0xFF) && Timer1);
...
static void sdcard_sendDataBlockBegin(uint8_t *buffer, bool multiBlockWrite)
{
    ...
    SD_TxDataBlock(&token, sizeof(token));
    SD_TxDataBlock(buffer, SDCARD_NON_DMA_CHUNK_SIZE);
...
static void sdcardSpi_init()
{
    ...
    while(!__HAL_SPI_GET_FLAG(&hspi2, SPI_FLAG_TXE));
	HAL_SPI_TransmitReceive(&hspi2, &dummy, &data, SDCARD_INIT_NUM_DUMMY_BYTES, 100);
    ...
    switch (sdcard.state) {
        ...
        case SDCARD_STATE_INITIALIZATION_RECEIVE_CID:
            if (sdcard_receiveCID()) {
                ...
                SPI_Set_Speed(_DEF_SPI2, SPI_BAUDRATEPRESCALER_2);
